...
public class TokenAuthenticationEntryPointTest {
    ...
    public void testTriggerMfa() throws JsonProcessingException {
        ...
        Assertions.assertEquals(WellKnownChallengeType.GOOGLE_AUTHENTICATOR, details.get("authenticator"));
        ...
        try {
            ...
            Assertions.assertEquals(
                    messageSource.getMessage(InvalidChallengeException.class, Locale.getDefault()),
                    oAuth2Error.getErrorDescription()
            );
    ...
    public static class UserIdSingleAccessToken implements OAuth2AuthorizationIdModifier {
        @Override
        public String modify(String originalId, MultiValueMap<String, String> headers, String clientGroup, @Nullable Client client, User user) {
            if (((ServletRequestAttributes) RequestContextHolder.getRequestAttributes()).getRequest().getParameter("singleAccessToken") != null) {
                return String.format("%s.%s", clientGroup, user.getId());
            } else {
                return originalId;
            }
        }
