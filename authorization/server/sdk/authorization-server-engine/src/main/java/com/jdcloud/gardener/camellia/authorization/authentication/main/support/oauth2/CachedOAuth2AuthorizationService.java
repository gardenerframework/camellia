...
import com.jdcloud.gardener.fragrans.data.cache.serialize.JdkSerializer;
import com.jdcloud.gardener.fragrans.data.cache.serialize.LongSerializer;
...
import com.jdcloud.gardener.fragrans.log.common.schema.verb.Delete;
import com.jdcloud.gardener.fragrans.log.common.schema.verb.Read;
import com.jdcloud.gardener.fragrans.log.common.schema.verb.Start;
...
public class CachedOAuth2AuthorizationService implements OAuth2AuthorizationService {
    ...
    private void loadQueryScript() throws IOException {
        if (this.queryScript == null || !cacheClient.scriptExists(this.queryScript)) {
            this.queryScript = this.cacheClient.loadLuaScriptFile("authorization-server-engine/scripts/oauth2-authorization-service/read-by-index.lua");
        }
    }
    ...
    private void loadUpdateScript() throws IOException {
        if (this.updateScript == null || !this.cacheClient.scriptExists(updateScript)) {
            this.updateScript = this.cacheClient.loadLuaScriptFile("authorization-server-engine/scripts/oauth2-authorization-service/update.lua");
        }
    }

    private void loadDeleteScript() throws IOException {
        if (this.deleteScript == null || !this.cacheClient.scriptExists(deleteScript)) {
            this.deleteScript = this.cacheClient.loadLuaScriptFile("authorization-server-engine/scripts/oauth2-authorization-service/remove.lua");
        }
    }
