...
public abstract class QueryCriteriaUtils {
     ...
     * @param trait                 字段trait
     */
    public static void addEqualsCriteria(Map<Class<?>, DatabaseCriteria> mappings, Class<?> entityType, String criteriaParameterName, Class<?> trait) {
        mappings.put(trait, new EqualsCriteria(
                FieldScanner.getInstance().column(entityType, trait),
                new FieldNameValue(criteriaParameterName, FieldScanner.getInstance().field(entityType, trait))
        ));
    }
    ...
    public static MatchAllCriteria createQueryCriteria(
            Supplier<Map<Class<?>, DatabaseCriteria>> mappingFactory,
            @Nullable Collection<Class<?>> must,
            @Nullable Collection<Class<?>> should
    ) {
        Map<Class<?>, DatabaseCriteria> mappings = mappingFactory.get();
        ...
        if (!CollectionUtils.isEmpty(must)) {
            ...
            must.forEach(
                    trait -> {
                        DatabaseCriteria element = mappings.get(trait);
                        if (element != null) {
